...
autoencoder_model <- luz::setup(
  module = autoencoder_model,
  loss = nn_mse_loss(),
  optimizer = optim_adam
)

autoencoder_constrained <- add_constraints(
  autoencoder_model,
  type = "l1_norm"
)
...
p <- ggplot(loss_df, aes(x = Epoch)) +
  geom_line(
    aes(y = Train_MSE, color = "Train MSE"),
    linewidth = 1.6
  ) +
  geom_line(
    aes(y = Validation_MSE, color = "Validation MSE"),
    linewidth = 1.6
  ) +
  coord_fixed(
...
ggsave(
  filename= "C:/Users/belen/OneDrive/Escritorio/TFM ISA/val_train_test.png",
  plot = p,
  width = 8,
  height = 6,
  dpi = 300,
